# -*- coding: utf-8 -*-
"""
Created on Thu Oct  8 09:28:56 2020
...
def P(k,n): #Poisson distribution
    ...
    #merge states, only consider k=0 and k=1
    if k == 0:
        return np.exp(-n)
    else:
        return 1. - np.exp(-n)
...
def psk2(k1, k2, R1, R2, t, sig1, sig2, s):
    ...
    return (Hs1*P(k1,R1)+(1-Hs1)*P(k1,0)) * (Hs2*P(k2,R2)+(1-Hs2)*P(k2,0))

def I2ON(t, sig1, sig2, R1, R2, ss):
    K = 2 #spike count over which is summed, merge states
    I2k = 0.
...
def mean_firing(t, sig1, sig2, R1, R2, ss):
    ...
    return np.trapz(P_s(ss) * (Hs1*R1 + Hs2*R2), ss)
...
np.savez(str(endtime), sig1=sig1, sig2=sig2, Nmax1=Nmax1, Nmax2=Nmax2, 
         N=N, Icontour=Icontour, avg_frate=avg_frate)
