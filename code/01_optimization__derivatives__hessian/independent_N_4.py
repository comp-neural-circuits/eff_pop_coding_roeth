...
'''
As with independent.py, but for N=4 neurons (just to demonstrate how to change N)
...
N = 4     #number of neurons. Attention!: Also number of loops and factors have to be adjusted in the function Info_3N_ind  (in specific: P_comb = Pk[:,0,i1]*Pk[:,1,i2]*Pk[:,2,i3]) )
...
def Info_4N_ind(t,sig,R,smin,smax,Ntrap):  
    #t: threshold vector
    #R: output noise
    #sig: input noise
    ...
    for n in range(0,N):   #for each neuron n: calculate H, Q, R; store Q and R in S
        H[:,n] = 0.5*erfc((t[n]-ss)/sig/np.sqrt(2)) #Eq. 9 from paper
        ...
        for i2 in range(0,2):
            for i3 in range(0,2):
                for i4 in range(0,2): 
                    ...
                    P_comb = Pk[:,0,i1]*Pk[:,1,i2]*Pk[:,2,i3]*Pk[:,3,i4] #The part over which is summed on the left hand side of Eq. 8 from paper; number of factors and loops has to be adjusted with N
...
def opt_Info_sig(sig): #do the optimization for a vector of output noises for a given sigma
    ...
    for i in range(0,len(R_vec)):
        ...
        t= scipy.optimize.minimize(Info_4N_ind,x0=t00,args=(sig,R,smin,smax,Ntrap), 
                                   method='nelder-mead', options={'xtol': xtol, 'ftol': ftol, 'disp': False})  #obtaining optimal trehsholds
      
        t00 = np.sort(t.x) #for avoiding local minima, but this does not work around discontinuous bifurcations

        
        I_j[i] = Info_4N_ind(t.x,sig,R,smin,smax,Ntrap)  #max information
        t_j[i,:] = np.sort(t.x)                        #optimal thresholds 
        
        #calculate Hesse matrix of information landscape
        H= nd.Hessian(Info_4N_ind)(t.x,sig,R,smin,smax,Ntrap)
